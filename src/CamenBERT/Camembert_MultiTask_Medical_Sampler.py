...
# Import Camembert model and tokenizer
from transformers import CamembertModel, CamembertTokenizer, get_linear_schedule_with_warmup
...
# ====================================================================
# 1. Dataset Class (added method to return all labels for sampling convenience)
# ====================================================================
...
class DatasetMultiTache(Dataset):
    ...
    def get_all_labels(self, label_type):
        """Returns a list of all labels for the specified task, used to calculate sampling weights."""
...
# ====================================================================
# 2. Multi-task Model (using CamembertModel)
# ====================================================================

class Camembert_MedicalMultiTache(nn.Module):
    def __init__(self, nom_modele_pre_entraine, num_intention, num_objet_medical, num_sentiment):
        super(Camembert_MedicalMultiTache, self).__init__()
        
        # Use CamembertModel as shared encoder
        self.camembert = CamembertModel.from_pretrained(nom_modele_pre_entraine)
        
        # Independent classification heads
        self.classifieur_intention = nn.Linear(self.camembert.config.hidden_size, num_intention)
        self.classifieur_objet_medical = nn.Linear(self.camembert.config.hidden_size, num_objet_medical)
        self.classifieur_sentiment = nn.Linear(self.camembert.config.hidden_size, num_sentiment)
        
    def forward(self, input_ids, attention_mask):
        # Through Camembert encoder
        outputs = self.camembert(input_ids=input_ids, attention_mask=attention_mask)
        
        # Use pooled output (usually the representation of the [CLS] token)
...
# ====================================================================
# 5. Main Program (Applying Camembert and sampling for Medical Object)
# ====================================================================

if __name__ == '__main__':
    NOM_MODELE_CAMEMBERT = 'camembert-base'
    MAX_LONGUEUR = 128
    TAILLE_DE_LOT = 16
    EPOQUES = 3
    ...
    tokenizer = CamembertTokenizer.from_pretrained(NOM_MODELE_CAMEMBERT)
    ...
    # --- Apply WeightedRandomSampler to the Medical Object task ---
    print("\n--- Calculating class weights for medical object classification for oversampling ---")
    ...
    print("\n--- Experiment Mode: Camembert + WeightedRandomSampler (Medical Object) ---")
    modele = Camembert_MedicalMultiTache(NOM_MODELE_CAMEMBERT, num_intention, num_objet_medical, num_sentiment).to(appareil)
    ...
    chemin_sauvegarde_modele = 'Camembert_MedicalMultiTache_sampler_objet_medical.pth'
