# Experiment Results Summary

**Labels:** NA_CATEGORY, diagnostique, symptome, traitement

## TF-IDF_ngram_LogisticRegression_Enhanced

- **Weighted F1 Score:** 0.6173
- **Accuracy:** 0.6413
- **Best Parameters:** {'classifier__C': 10, 'classifier__solver': 'liblinear'}
```json
{
    "NA_CATEGORY": {
        "precision": 0.8888888888888888,
        "recall": 0.6666666666666666,
        "f1-score": 0.7619047619047619,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.8,
        "recall": 0.21052631578947367,
        "f1-score": 0.3333333333333333,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.6585365853658537,
        "recall": 0.8181818181818182,
        "f1-score": 0.7297297297297297,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.5405405405405406,
        "recall": 0.7142857142857143,
        "f1-score": 0.6153846153846154,
        "support": 28.0
    },
    "accuracy": 0.6413043478260869,
    "macro avg": {
        "precision": 0.7219915036988207,
        "recall": 0.6024151287309182,
        "f1-score": 0.61008811008811,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.6818859686834237,
        "recall": 0.6413043478260869,
        "f1-score": 0.6172613129134868,
        "support": 92.0
    }
}
```

## TF-IDF_LogisticRegression_Enhanced

- **Weighted F1 Score:** 0.5981
- **Accuracy:** 0.5978
- **Best Parameters:** {'classifier__C': 10, 'classifier__solver': 'liblinear'}
```json
{
    "NA_CATEGORY": {
        "precision": 1.0,
        "recall": 0.6666666666666666,
        "f1-score": 0.8,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.5,
        "recall": 0.3157894736842105,
        "f1-score": 0.3870967741935484,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.7058823529411765,
        "recall": 0.7272727272727273,
        "f1-score": 0.7164179104477612,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.4473684210526316,
        "recall": 0.6071428571428571,
        "f1-score": 0.5151515151515151,
        "support": 28.0
    },
    "accuracy": 0.5978260869565217,
    "macro avg": {
        "precision": 0.663312693498452,
        "recall": 0.5792179311916154,
        "f1-score": 0.6046665499482061,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.6230481895275273,
        "recall": 0.5978260869565217,
        "f1-score": 0.5980529584640866,
        "support": 92.0
    }
}
```

## BoW_SVM_Enhanced

- **Weighted F1 Score:** 0.5652
- **Accuracy:** 0.5652
- **Best Parameters:** {'classifier__C': 1, 'classifier__kernel': 'linear'}
```json
{
    "NA_CATEGORY": {
        "precision": 0.7272727272727273,
        "recall": 0.6666666666666666,
        "f1-score": 0.6956521739130435,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.5333333333333333,
        "recall": 0.42105263157894735,
        "f1-score": 0.47058823529411764,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.6129032258064516,
        "recall": 0.5757575757575758,
        "f1-score": 0.59375,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.4857142857142857,
        "recall": 0.6071428571428571,
        "f1-score": 0.5396825396825397,
        "support": 28.0
    },
    "accuracy": 0.5652173913043478,
    "macro avg": {
        "precision": 0.5898058930316995,
        "recall": 0.5676549327865117,
        "f1-score": 0.5749182372224252,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.5726783968719452,
        "recall": 0.5652173913043478,
        "f1-score": 0.5651506920506072,
        "support": 92.0
    }
}
```

## TF-IDF_SVM_Enhanced

- **Weighted F1 Score:** 0.5526
- **Accuracy:** 0.5435
- **Best Parameters:** {'classifier__C': 10, 'classifier__kernel': 'linear'}
```json
{
    "NA_CATEGORY": {
        "precision": 1.0,
        "recall": 0.6666666666666666,
        "f1-score": 0.8,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.42105263157894735,
        "recall": 0.42105263157894735,
        "f1-score": 0.42105263157894735,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.6333333333333333,
        "recall": 0.5757575757575758,
        "f1-score": 0.6031746031746031,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.42857142857142855,
        "recall": 0.5357142857142857,
        "f1-score": 0.47619047619047616,
        "support": 28.0
    },
    "accuracy": 0.5434782608695652,
    "macro avg": {
        "precision": 0.6207393483709273,
        "recall": 0.5497977899293688,
        "f1-score": 0.5751044277360067,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.575,
        "recall": 0.5434782608695652,
        "f1-score": 0.5525879917184264,
        "support": 92.0
    }
}
```

## BoW_LogisticRegression_Enhanced

- **Weighted F1 Score:** 0.5516
- **Accuracy:** 0.5543
- **Best Parameters:** {'classifier__C': 10, 'classifier__solver': 'liblinear'}
```json
{
    "NA_CATEGORY": {
        "precision": 0.8,
        "recall": 0.6666666666666666,
        "f1-score": 0.7272727272727273,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.5454545454545454,
        "recall": 0.3157894736842105,
        "f1-score": 0.4,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.6060606060606061,
        "recall": 0.6060606060606061,
        "f1-score": 0.6060606060606061,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.4473684210526316,
        "recall": 0.6071428571428571,
        "f1-score": 0.5151515151515151,
        "support": 28.0
    },
    "accuracy": 0.5543478260869565,
    "macro avg": {
        "precision": 0.5997208931419458,
        "recall": 0.5489149008885851,
        "f1-score": 0.5621212121212121,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.5705429581859789,
        "recall": 0.5543478260869565,
        "f1-score": 0.5516469038208168,
        "support": 92.0
    }
}
```

## TF-IDF_ngram_SVM_Enhanced

- **Weighted F1 Score:** 0.5396
- **Accuracy:** 0.5435
- **Best Parameters:** {'classifier__C': 10, 'classifier__kernel': 'linear'}
```json
{
    "NA_CATEGORY": {
        "precision": 0.875,
        "recall": 0.5833333333333334,
        "f1-score": 0.7,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.4166666666666667,
        "recall": 0.2631578947368421,
        "f1-score": 0.3225806451612903,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.6176470588235294,
        "recall": 0.6363636363636364,
        "f1-score": 0.6268656716417911,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.4473684210526316,
        "recall": 0.6071428571428571,
        "f1-score": 0.5151515151515151,
        "support": 28.0
    },
    "accuracy": 0.5434782608695652,
    "macro avg": {
        "precision": 0.589170536635707,
        "recall": 0.5224994303941672,
        "f1-score": 0.5411494579886491,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.5578840804056177,
        "recall": 0.5434782608695652,
        "f1-score": 0.5395634983313701,
        "support": 92.0
    }
}
```

## TF-IDF_ngram_KNN_Enhanced

- **Weighted F1 Score:** 0.5066
- **Accuracy:** 0.5109
- **Best Parameters:** {'classifier__n_neighbors': 7}
```json
{
    "NA_CATEGORY": {
        "precision": 0.7777777777777778,
        "recall": 0.5833333333333334,
        "f1-score": 0.6666666666666666,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.45454545454545453,
        "recall": 0.2631578947368421,
        "f1-score": 0.3333333333333333,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.5625,
        "recall": 0.5454545454545454,
        "f1-score": 0.5538461538461539,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.425,
        "recall": 0.6071428571428571,
        "f1-score": 0.5,
        "support": 28.0
    },
    "accuracy": 0.5108695652173914,
    "macro avg": {
        "precision": 0.554955808080808,
        "recall": 0.49977215766689453,
        "f1-score": 0.5134615384615384,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.5264369235836627,
        "recall": 0.5108695652173914,
        "f1-score": 0.5066332218506132,
        "support": 92.0
    }
}
```

## TF-IDF_ngram_DecisionTree_Enhanced

- **Weighted F1 Score:** 0.5020
- **Accuracy:** 0.5000
- **Best Parameters:** {'classifier__max_depth': None}
```json
{
    "NA_CATEGORY": {
        "precision": 0.7777777777777778,
        "recall": 0.5833333333333334,
        "f1-score": 0.6666666666666666,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.4444444444444444,
        "recall": 0.42105263157894735,
        "f1-score": 0.43243243243243246,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.5277777777777778,
        "recall": 0.5757575757575758,
        "f1-score": 0.5507246376811594,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.41379310344827586,
        "recall": 0.42857142857142855,
        "f1-score": 0.42105263157894735,
        "support": 28.0
    },
    "accuracy": 0.5,
    "macro avg": {
        "precision": 0.540948275862069,
        "recall": 0.5021787423103212,
        "f1-score": 0.5177190920898014,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.5084853406630018,
        "recall": 0.5,
        "f1-score": 0.501952205912011,
        "support": 92.0
    }
}
```

## TF-IDF_DecisionTree_Enhanced

- **Weighted F1 Score:** 0.4982
- **Accuracy:** 0.5000
- **Best Parameters:** {'classifier__max_depth': None}
```json
{
    "NA_CATEGORY": {
        "precision": 0.8,
        "recall": 0.6666666666666666,
        "f1-score": 0.7272727272727273,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.4,
        "recall": 0.42105263157894735,
        "f1-score": 0.41025641025641024,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.5128205128205128,
        "recall": 0.6060606060606061,
        "f1-score": 0.5555555555555556,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.43478260869565216,
        "recall": 0.35714285714285715,
        "f1-score": 0.39215686274509803,
        "support": 28.0
    },
    "accuracy": 0.5,
    "macro avg": {
        "precision": 0.5369007803790413,
        "recall": 0.5127306903622693,
        "f1-score": 0.5213103889574479,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.5032281518103824,
        "recall": 0.5,
        "f1-score": 0.4982159783950066,
        "support": 92.0
    }
}
```

## TF-IDF_KNN_Enhanced

- **Weighted F1 Score:** 0.4915
- **Accuracy:** 0.4891
- **Best Parameters:** {'classifier__n_neighbors': 7}
```json
{
    "NA_CATEGORY": {
        "precision": 0.75,
        "recall": 0.5,
        "f1-score": 0.6,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.35294117647058826,
        "recall": 0.3157894736842105,
        "f1-score": 0.3333333333333333,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.5806451612903226,
        "recall": 0.5454545454545454,
        "f1-score": 0.5625,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.4166666666666667,
        "recall": 0.5357142857142857,
        "f1-score": 0.46875,
        "support": 28.0
    },
    "accuracy": 0.4891304347826087,
    "macro avg": {
        "precision": 0.5250632511068943,
        "recall": 0.4742395762132604,
        "f1-score": 0.49114583333333334,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.5058026015455271,
        "recall": 0.4891304347826087,
        "f1-score": 0.49153079710144926,
        "support": 92.0
    }
}
```

## BoW_DecisionTree_Enhanced

- **Weighted F1 Score:** 0.4722
- **Accuracy:** 0.4783
- **Best Parameters:** {'classifier__max_depth': None}
```json
{
    "NA_CATEGORY": {
        "precision": 0.7777777777777778,
        "recall": 0.5833333333333334,
        "f1-score": 0.6666666666666666,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.3076923076923077,
        "recall": 0.21052631578947367,
        "f1-score": 0.25,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.475,
        "recall": 0.5757575757575758,
        "f1-score": 0.5205479452054794,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.4666666666666667,
        "recall": 0.5,
        "f1-score": 0.4827586206896552,
        "support": 28.0
    },
    "accuracy": 0.4782608695652174,
    "macro avg": {
        "precision": 0.5067841880341881,
        "recall": 0.4674043062200957,
        "f1-score": 0.4799933081404503,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.4774038461538461,
        "recall": 0.4782608695652174,
        "f1-score": 0.4722317779466431,
        "support": 92.0
    }
}
```

## BoW_KNN_Enhanced

- **Weighted F1 Score:** 0.4451
- **Accuracy:** 0.5000
- **Best Parameters:** {'classifier__n_neighbors': 5}
```json
{
    "NA_CATEGORY": {
        "precision": 0.5,
        "recall": 0.8333333333333334,
        "f1-score": 0.625,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.5,
        "recall": 0.21052631578947367,
        "f1-score": 0.2962962962962963,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.4909090909090909,
        "recall": 0.8181818181818182,
        "f1-score": 0.6136363636363636,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.5555555555555556,
        "recall": 0.17857142857142858,
        "f1-score": 0.2702702702702703,
        "support": 28.0
    },
    "accuracy": 0.5,
    "macro avg": {
        "precision": 0.5116161616161616,
        "recall": 0.5101532239690134,
        "f1-score": 0.45130073255073255,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.5136473429951691,
        "recall": 0.5,
        "f1-score": 0.4450782304043174,
        "support": 92.0
    }
}
```

## Word2Vec_KNN_Enhanced

- **Weighted F1 Score:** 0.4496
- **Accuracy:** 0.4891
- **Best Parameters:** {'n_neighbors': 7}
```json
{
    "NA_CATEGORY": {
        "precision": 0.8571428571428571,
        "recall": 0.5,
        "f1-score": 0.631578947368421,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.3333333333333333,
        "recall": 0.3684210526315789,
        "f1-score": 0.35,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.5185185185185185,
        "recall": 0.8484848484848485,
        "f1-score": 0.6436781609195402,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.4,
        "recall": 0.14285714285714285,
        "f1-score": 0.21052631578947367,
        "support": 28.0
    },
    "accuracy": 0.4891304347826087,
    "macro avg": {
        "precision": 0.5272486772486772,
        "recall": 0.46494076099339254,
        "f1-score": 0.4589458560193587,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.4883712905452036,
        "recall": 0.4891304347826087,
        "f1-score": 0.4496202556616428,
        "support": 92.0
    }
}
```

## FastText_KNN_Enhanced

- **Weighted F1 Score:** 0.3948
- **Accuracy:** 0.4239
- **Best Parameters:** {'n_neighbors': 5}
```json
{
    "NA_CATEGORY": {
        "precision": 0.8571428571428571,
        "recall": 0.5,
        "f1-score": 0.631578947368421,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.28205128205128205,
        "recall": 0.5789473684210527,
        "f1-score": 0.3793103448275862,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.5128205128205128,
        "recall": 0.6060606060606061,
        "f1-score": 0.5555555555555556,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.2857142857142857,
        "recall": 0.07142857142857142,
        "f1-score": 0.11428571428571428,
        "support": 28.0
    },
    "accuracy": 0.42391304347826086,
    "macro avg": {
        "precision": 0.4844322344322344,
        "recall": 0.4391091364775575,
        "f1-score": 0.42018264050931925,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.44095397356266913,
        "recall": 0.42391304347826086,
        "f1-score": 0.3947736657986797,
        "support": 92.0
    }
}
```

## FastText_DecisionTree_Enhanced

- **Weighted F1 Score:** 0.3918
- **Accuracy:** 0.3913
- **Best Parameters:** {'max_depth': 10}
```json
{
    "NA_CATEGORY": {
        "precision": 0.5333333333333333,
        "recall": 0.6666666666666666,
        "f1-score": 0.5925925925925926,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.2857142857142857,
        "recall": 0.3157894736842105,
        "f1-score": 0.3,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.5,
        "recall": 0.3939393939393939,
        "f1-score": 0.4406779661016949,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.3,
        "recall": 0.32142857142857145,
        "f1-score": 0.3103448275862069,
        "support": 28.0
    },
    "accuracy": 0.391304347826087,
    "macro avg": {
        "precision": 0.40476190476190477,
        "recall": 0.42445602642971064,
        "f1-score": 0.4109038465701236,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.39922360248447203,
        "recall": 0.391304347826087,
        "f1-score": 0.391773251792183,
        "support": 92.0
    }
}
```

## Word2Vec_DecisionTree_Enhanced

- **Weighted F1 Score:** 0.3584
- **Accuracy:** 0.3587
- **Best Parameters:** {'max_depth': 10}
```json
{
    "NA_CATEGORY": {
        "precision": 0.45454545454545453,
        "recall": 0.4166666666666667,
        "f1-score": 0.43478260869565216,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.29411764705882354,
        "recall": 0.2631578947368421,
        "f1-score": 0.2777777777777778,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.3870967741935484,
        "recall": 0.36363636363636365,
        "f1-score": 0.375,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.3333333333333333,
        "recall": 0.39285714285714285,
        "f1-score": 0.36065573770491804,
        "support": 28.0
    },
    "accuracy": 0.358695652173913,
    "macro avg": {
        "precision": 0.3672733022827899,
        "recall": 0.3590795169742538,
        "f1-score": 0.362054031044587,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.36032943076503837,
        "recall": 0.358695652173913,
        "f1-score": 0.35835358410720985,
        "support": 92.0
    }
}
```

## Word2Vec_LogisticRegression_Enhanced

- **Weighted F1 Score:** 0.1894
- **Accuracy:** 0.3587
- **Best Parameters:** {'C': 0.1, 'solver': 'liblinear'}
```json
{
    "NA_CATEGORY": {
        "precision": 0.0,
        "recall": 0.0,
        "f1-score": 0.0,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.0,
        "recall": 0.0,
        "f1-score": 0.0,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.358695652173913,
        "recall": 1.0,
        "f1-score": 0.528,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.0,
        "recall": 0.0,
        "f1-score": 0.0,
        "support": 28.0
    },
    "accuracy": 0.358695652173913,
    "macro avg": {
        "precision": 0.08967391304347826,
        "recall": 0.25,
        "f1-score": 0.132,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.1286625708884688,
        "recall": 0.358695652173913,
        "f1-score": 0.18939130434782608,
        "support": 92.0
    }
}
```

## Word2Vec_SVM_Enhanced

- **Weighted F1 Score:** 0.1894
- **Accuracy:** 0.3587
- **Best Parameters:** {'C': 0.1, 'kernel': 'linear'}
```json
{
    "NA_CATEGORY": {
        "precision": 0.0,
        "recall": 0.0,
        "f1-score": 0.0,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.0,
        "recall": 0.0,
        "f1-score": 0.0,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.358695652173913,
        "recall": 1.0,
        "f1-score": 0.528,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.0,
        "recall": 0.0,
        "f1-score": 0.0,
        "support": 28.0
    },
    "accuracy": 0.358695652173913,
    "macro avg": {
        "precision": 0.08967391304347826,
        "recall": 0.25,
        "f1-score": 0.132,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.1286625708884688,
        "recall": 0.358695652173913,
        "f1-score": 0.18939130434782608,
        "support": 92.0
    }
}
```

## FastText_LogisticRegression_Enhanced

- **Weighted F1 Score:** 0.1894
- **Accuracy:** 0.3587
- **Best Parameters:** {'C': 0.1, 'solver': 'liblinear'}
```json
{
    "NA_CATEGORY": {
        "precision": 0.0,
        "recall": 0.0,
        "f1-score": 0.0,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.0,
        "recall": 0.0,
        "f1-score": 0.0,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.358695652173913,
        "recall": 1.0,
        "f1-score": 0.528,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.0,
        "recall": 0.0,
        "f1-score": 0.0,
        "support": 28.0
    },
    "accuracy": 0.358695652173913,
    "macro avg": {
        "precision": 0.08967391304347826,
        "recall": 0.25,
        "f1-score": 0.132,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.1286625708884688,
        "recall": 0.358695652173913,
        "f1-score": 0.18939130434782608,
        "support": 92.0
    }
}
```

## FastText_SVM_Enhanced

- **Weighted F1 Score:** 0.1894
- **Accuracy:** 0.3587
- **Best Parameters:** {'C': 0.1, 'kernel': 'linear'}
```json
{
    "NA_CATEGORY": {
        "precision": 0.0,
        "recall": 0.0,
        "f1-score": 0.0,
        "support": 12.0
    },
    "diagnostique": {
        "precision": 0.0,
        "recall": 0.0,
        "f1-score": 0.0,
        "support": 19.0
    },
    "symptome": {
        "precision": 0.358695652173913,
        "recall": 1.0,
        "f1-score": 0.528,
        "support": 33.0
    },
    "traitement": {
        "precision": 0.0,
        "recall": 0.0,
        "f1-score": 0.0,
        "support": 28.0
    },
    "accuracy": 0.358695652173913,
    "macro avg": {
        "precision": 0.08967391304347826,
        "recall": 0.25,
        "f1-score": 0.132,
        "support": 92.0
    },
    "weighted avg": {
        "precision": 0.1286625708884688,
        "recall": 0.358695652173913,
        "f1-score": 0.18939130434782608,
        "support": 92.0
    }
}
```

